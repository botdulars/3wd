...
void setup() {
  ...
  Serial.begin(9600) ; 
  Serial.println("Welcome to the botdulars final program for Levels 1&2. Use numbers 1 to 10 to test each Component") ; 
...
void loop() {
  ...
  //if (Serial.available() && (value>0) )
  {
    ...
    Serial.print("Received input from serial as:") ; 
    Serial.println(value) ; 
  }
    ...
    else if (value == 9) turnONBuzzer() ;
    else if (value == 10) showButtonstate() ;
    else if (value == 11) showIRstate() ;
...
void showPotValue() 
{
  int potval = analogRead(A3);          //Read and save analog value from potentiometer
...
void turnONGreenLED() 
{
    ...
    Serial.println("This instruction turns the green LED ON for 5 sec") ; 
    delay(5000); 
...
void turnONBuzzer() 
{
    ...
    Serial.println("This instruction turns the Buzzer ON for 5 sec") ; 
    delay(5000); 
...
{
  int Buttonstate = digitalRead(12);//Read and save analog value from potentiometer
